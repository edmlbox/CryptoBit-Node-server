...
app.post('/coinmarket/icons/:icoIdArr', (req, res) => {
    ...
    fetch('https://pro-api.coinmarketcap.com/v1/cryptocurrency/info?id=' + getArray, {
            method: 'GET',
            headers: {
...
app.post('/pincode',(req,res)=>{
    ...
    req.on('data', (chunk) => {
        ...
        MongoClient.connect(url, (err, client) => {
            client.db('cryptobit').collection('col999').findOneAndUpdate({email:bodyObj.email}, { $set: {lockPassword:bodyObj.pincode}},(err,result)=>{
...
app.post('/update', (req, res) => {
    ...
    req.on('data', (chunk) => {
        ...
        MongoClient.connect(url, (err, client) => {
           ...
           client.db('cryptobit').collection('col999').updateOne(
...
function findInData(bodyObj, req, res) {
    const url = 'mongodb://mariah:159753zaq@ds115553.mlab.com:15553/cryptobit';
    MongoClient.connect(url, (err, client) => {
        client.db('cryptobit').collection('col999').findOne({
            email: bodyObj.email
        }, (err, result) => {
            console.log(result)
    ...
    MongoClient.connect(url, (err, client) => {
        ...
        client.db('cryptobit').collection('col999').findOne({
            ...
            if (response) {
                ...
                client.db('cryptobit').collection('col999').insertOne({
                    email: dataxxx.email,
